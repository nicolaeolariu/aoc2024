...
const machinesInt = machines.map(machine => {
  const [buttonA, buttonB, prize] = machine;
  return machine.map(row => row.split(": ")[1].split(', ').map(a => {
    if(a.indexOf('X+') === 0 || a.indexOf('Y+') === 0) {
      return parseInt(a.split('+')[1])
    }

    if(a.indexOf('X=') === 0 || a.indexOf('Y=') === 0) {
      return parseInt(a.split('=')[1]) + 10000000000000;
    }
  }))
})

console.log(machinesInt)
const calculatePrice = ({A, B}) => {
  return A * 3 + B;
}

const calculateFirst = (prize, first, second) => {
  return (prize[1] * first[0] - prize[0] * first[1]) / (first[0] * second[1] - first[1] * second[0])
}

const calculateSecond = (prize, first, second, firstValue) => {
  return (prize[0] - second[0] * firstValue) / first[0]
}

let pressed = {};

machinesInt.forEach((machine, k) => {
  const [buttonA, buttonB, prize] = machine;
  console.log('buttonA', buttonA,  'buttonB', buttonB, 'prize', prize);
  const y = calculateFirst(prize, buttonA, buttonB); //(prize[1] * buttonB[0] - prize[0] * buttonB[1]) / (buttonB[0] * buttonA[1] - buttonB[1] * buttonA[0]);
  const x = calculateSecond(prize, buttonA, buttonB, y);  //(prize[0] - buttonA[0] * a) / buttonB[0];
  if(y === parseInt(y) && x === parseInt(x)) {
    if(buttonA[0] * x + buttonB[0] * y === prize[0] && buttonA[1] * x + buttonB[1] * y === prize[1]) { // Double check the sum
      pressed[k] = pressed[k] || [];
      pressed[k].push({A: Math.abs(x), B: Math.abs(y)})
    }
   }
});
...
console.log(pressed);
for(let machine in pressed) {
  const tokens = calculatePrice(pressed[machine][0]);
  console.log('for:', machine, 'tokens:', tokens)
  totalTokens += tokens
}

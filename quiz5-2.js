...
const rules = rulesInput.map(row => row.split('|'));
const updates = updateInput.map(row=> row.split(','));

const getRules = (number, key) => {
  return rules.filter(row => row[key] === number);
}
...
updates.forEach(row => {
  ...
  row.forEach((number, k) => {
    const rulesBefore = getRules(number, 0)
    const rulesAfter = getRules(number, 1)

    rulesBefore.forEach(rule => {
      const position = row.indexOf(rule[1]);
      if(position !== -1 && k > position) {
        isValid = false;
      }
    });
    rulesAfter.forEach(rule => {
      const position = row.indexOf(rule[0]);
      if(position !== -1 && k < position) {
        isValid = false;
      }
    });
  ...
  if(!isValid) {
    ...
    const correctOrder = sortRow(row);
    console.log('output', correctOrder)
    let middleKey = correctOrder[Math.round((correctOrder.length - 1) / 2)];
    console.log(middleKey)
    sum += parseInt(middleKey);
  }
});
